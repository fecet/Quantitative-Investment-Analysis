...
> The covariance of the return on the global minimum variance (GMV) portfolio and that on any portfolio (not necessary on the frontier) is always equal to $\sigma_{mv}^2=\frac{1}{\delta}$.

> The covariance of the return on a frontier portfolio $q$ and that on any portfolio $p$ (not on the frontier) with the same expected return as $q$ is always equal to the variance of the frontier portfolio $q$. Formally $E[\mathbf{r_p}]=E[\mathbf{r_q}]\implies \text{Cov}(\mathbf{r_p,r_q})=\sigma_q^2$.

Now suppose $r_p$ is any portfolio and $r_q$ on the froniter with $\mathop{\text{E}}[r_q]=\mathop{\text{E}}[r_p]$. By first assertion, we decompose $r_p=r_{mv}+\epsilon$

$$\text{Cov}(\mathbf{r_p-r_{mv},r_{mv}})=\text{Cov}(\mathbf{r_p,r_{mv}})-\sigma_{mv}^2=0$$

We call $\epsilon=\mathbf{r_p-r_{mv}}$ **excess return**. And the second implies that we can decompose $\mathbf{r_p=r_q+\epsilon_p}$ where $E[\epsilon]=\text{Cov}(\epsilon_p,\mathbf{r_q})=0$
...
1. 
   ...
   when $\bm{\mathbf{\omega}}=\bm{\mathbf{\mathbf{V^-}(\overline{\mathbf{r}}-r_f\mathbf{e})}}$.
