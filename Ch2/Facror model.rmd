...
By the discussion above, $\alpha=\mathbf{0}$ when CAPM holds. Assume $\{\nu_t\}_{t=1}^T$ i.i.d with $\mathcal{N}(0,\Sigma)$, we have $\mathbf{r_t^e}\mid r_{m,t}^e\sim \mathcal{N}(\alpha+\beta r_{m,t}^e,\Sigma)$. 
...
Now $\mathbf{r'}\sim \mathcal{MN}_{n\times T}(\mathbf{\alpha e'+\beta r_m',\Sigma,I})$, the p.d.f is [@wiki:mnt]
...
FOC w.r.t $\alpha$, by chain rule[@petersen2012matrix]
...
Similarly to out deduction for mean-variance mdoel, let $a=\mathbf{r_m'r_m},b=\mathbf{e'e}=T$ and $c=\mathbf{e'r_m}$($c^2< ab$), hence
...
By assumption $\mathbf{r'}\sim \mathcal{MN}_{n\times T}(\mathbf{\beta r_m',\Sigma,I})$, and $\alpha=\mathbf{r'}(c\mathbf{r_m}-a\mathbf{e})/(c^2-ab)$

By transformation of matrix normal distribution[@wiki:mnt]
...
which degenerated to $\mathcal{N}(0,\mathbf{\frac{a}{ab-c^2}\Sigma})$ since $\Sigma \otimes \frac{a}{ab-c^2}=\frac{a}{ab-c^2}\Sigma$.[@wiki:mnt] For the same reason, $\hat{\beta}\sim\mathcal{N}(\mathbf{\beta},\frac{b}{ab-c^2}\Sigma)$


FOC w.r.t $\Sigma$[@petersen2012matrix]:
...
Easy to verify $\frac{c\mathbf{r_me'}-a\mathbf{ee'}-b\mathbf{r_mr_m'}+c\mathbf{er_m'}}{c^2-ab}$ is symmetric and idempotent, thus
...
We may find $\mathbf{r'P}\sim \mathcal{MN}_{n\times(T-2)}(\mathbf{0},\Sigma,\mathbf{I})$. Where $\ep[\mathbf{r'P}]=\ep[\mathbf{r'PP'P}]=\ep[\mathbf{r'AP}]=\mathbf{0}$ and thus [@wiki:wst]
